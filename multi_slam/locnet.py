...
class Update(nn.Module):
    ...
    def __init__(self, p, num_gru, corr_planes, grid_rad, num_heads, transf_dim):
        ...
        self.transformer = nn.Sequential(nn.Linear(DIM, transf_dim),
                                         nn.TransformerEncoderLayer(d_model=transf_dim, nhead=num_heads,
                                                                    batch_first=True, dropout=0.0),
                                         nn.Linear(transf_dim, DIM))

        self.gru = nn.Sequential(
            *[nn.Sequential(nn.LayerNorm(DIM, eps=1e-3), GatedResidual(DIM)) for _ in range(num_gru)])
...
class CorrBlock:
    ...
    def __call__(self, ii, jj, coords):
        ...
        for i in range(len(self.levels)):
            corrs += [
                altcorr.corr(self.gmap, self.pyramid[i], coords / self.levels[i], ii, jj, self.radius, self.dropout)]
        return corrs
...
def calc_gt_matches(pts1, depth1, depths, intrinsics, Ps, cycle_thresh=np.inf):
    ...
    sampling_pts2 = rearrange(pts2, 'B LR M uv -> (B LR) M 1 uv') * torch.tensor([2 / (W - 1), 2 / (H - 1)],
                                                                                 device='cuda') - 1
    sampled_depth2 = F.grid_sample(rearrange(depths[:, [1, 0]], 'B LR H W -> (B LR) 1 H W'), sampling_pts2,
                                   align_corners=True)
...
class LocNet(nn.Module):
    ...
    def forward(self, images, intrinsics, kp_or_detector, Ps=None, depths=None, bounds=None, M=None, STEPS=12,
                opt_iters=5, depth_conf=None, use_autocast=False, homography_pretrain=False):
        ...
        invalid_depth = (repeat(~val_depth, '... -> ... P1 P2 3', P1=p, P2=p)) & torch.tensor(
        ...
        if homography_pretrain:
            ...
            gt_matches, valid_repr_disp, cycle_consistency = calc_gt_matches(centroids * 8.0, 1 / patch_disps, depths,
                                                                             intrinsics, Ps)
            gt_matches /= 8.0
        else:
        ...
        patches = patches[..., :2]
        fixed = {"centroids": centroids * 8.0, "gt_matches": gt_matches * 8.0, "has_accurate_gt": has_accurate_gt,
                 "cycle_consistent_gt": cycle_consistency}
        ...
        for it in range(STEPS):
            ...
            if not homography_pretrain:
                try:
                    ...
                    Gs = epops.weighted_epa(pts1, pts2, dir_weightsX * 0.01, intr1, intr2,
                                            Ps[:, 1] if self.training else None, (hh, ww))
            ...
            predictions.append(
                {"matches": target * 8.0, "pre_update_matches": pre_update_matches * 8.0, "poses": Gs.inv(),
                 "gn_weight": gn_weights, "weight": dir_weights})  # returns pose in world

            if (depth_conf is not None) and (it == (STEPS - 1)):
                if isinstance(kp_or_detector, tuple):
                    _, _, M, _ = kp_or_detector[0].shape
                    return compute_sim3(Gs, centroids[:, :, :M], pre_update_matches[:, :, :M], intrinsics,
                                        patch_disps[:, :, :M], val_depth[:, :, :M], dir_weights[:, :, :M], depth_conf)
                return compute_sim3(Gs, centroids, pre_update_matches, intrinsics, patch_disps, val_depth, dir_weights,
                                    depth_conf)
